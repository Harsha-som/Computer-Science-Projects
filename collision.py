...
CS152
lab8
have code for collisin functions
change directory to project 8 and run py.collision.py to call
'''
# Bruce A. Maxwell
# CS 151S
# Fall 2015
...
def collisionTest_block_block(block1, block2):
    ...
    if tmin < 0 and tmax < 0: # both intersections behind the ball
        ...
        if v[0] == 0.0 and v[1] == 0.0:
            v = (1.0, 0.0)
            tmin = new_func(block1P, block2P, dx, dx2)
        ...
        if -tmin < tmax:
            ...
            #print("setting position using tmax and velocity %.2f %d" % (tmax, sidemax))
            block2.setPosition( block1P[0] + (tmax+1e-3)*v[0], block1P[1] + (tmax+1e-3)*v[1])
        tmin = 0
    ...
    # 3: coming from the right
    return (tmin, side)
def new_func(block1P, block2P, dx, dx2):
   tmin = (block1P[0] - 0.5*dx - dx2*0.5) - block2P[0]
   return tmin
