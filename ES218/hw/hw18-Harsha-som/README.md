...
In a single piping operation, can you generate a table in long form that
can then be used in `ggplot` to generate the following faceted table?
...
-   Because `a` is generated randomly each time the `runif` function is
    called, we need to create it just once, then replicate it in a
    dataframe. Since we will evaluate six quantile types, we will need
    six replicates.
...
-   You *can* store formulae as character variables. To convert a
    formula variable to an actual formula, use the `eval()` and
